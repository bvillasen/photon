...
def setCudaDevice( devN = None, usingAnimation=False, print_out=True  ):
  ...
  for i in range(nDevices):
    dev = cuda.Device( i )
    if print_out: print( "  Device {0}: {1}".format( i, dev.name() ))
  devNumber = 0
  if nDevices > 1:
    if devN == None:
  ...
  if usingAnimation:
    ...
    dev.make_context()
  if print_out: print( "Using device {0}: {1}".format( devNumber, dev.name() ))
